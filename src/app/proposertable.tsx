...
const ConsensusTable: React.FC<ConsensusTableProps> = ({ consensusAccounts: consensusAccountsProp }) => {
    ...
    // Manual order for headers to ensure q05Latency is shown after proposals
    ...
    const [nfdMappings, setNfdMappings] = useState<{ [address: string]: string }>({});
     ...
     const fetchNFDsForAddresses = (addresses: string[]) => {
        ...
        fetch(url)
            .then(response => {
                if (response.status === 404) throw new Error('No NFDs found.');
    ...
    useEffect(() => {
        ...
        return () => {
            // If you have any other clean-up operations, add them here.
        };
