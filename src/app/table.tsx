...
function truncateString(str: string | undefined, num: number): string {
  if (!str) return "";  // or provide a default value like "N/A"
...
export default function MyTable({ allNodes, setAllNodes }: Props) {
  ...
  function getCircleColor(nodeScore: number) {
    if (nodeScore >= 0 && nodeScore < 2) {
      return "red";
    } else if (nodeScore >= 2 && nodeScore < 5) {
      return "yellow";
    } else if (nodeScore >= 5 && nodeScore <= 10) {
      return "green";
    } else {
      return "gray";
    }  // default/fallback color for scores outside the provided range
  }
  ...
  useEffect(() => {
    const fetchData = async (endpoint: string, index: number) => {
      const response = await fetch(`https://voi-node-info.boeieruurd.com/api/node-health/${endpoint}`);
    ...
    <div className="mt-6">
      <Card>
        ...
        <div className="table-container">
          <Table className="hide-columns mt-0 p-0">
            <TableHead>
              <TableRow>
